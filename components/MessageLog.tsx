...
const MessageLog: React.FC<MessageLogProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-zinc-200 h-full flex flex-col overflow-hidden relative">
      {/* Toolbar */}
      <div className="px-3 py-1.5 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50 gap-3">
        <div className="flex items-center gap-3">
            <h3 className="text-sm font-bold text-zinc-700 flex items-center gap-2">
                <i className="fas fa-list-ul text-indigo-500"></i>
                {t('messageLog.title')}
                <span className="px-2 py-0.5 rounded-full bg-zinc-200 text-zinc-600 text-xs font-mono">{filteredMessages.length} <span className="text-zinc-400 font-normal">/ {messages.length}</span></span>
            </h3>
            ...
            <div className="relative group">
                ...
                <input 
                    ...
                    className="pl-7 pr-3 py-1 bg-white border border-zinc-200 rounded-full text-xs w-32 focus:w-48 transition-all focus:ring-2 focus:ring-indigo-100 outline-none"
                    value={filterText}
        ...
        <div className="flex items-center gap-2">
            <button 
                ...
                className={`px-2 py-1 rounded text-xs font-medium flex items-center gap-1 transition-colors ${isPaused ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'text-zinc-500 hover:bg-zinc-100'}`}
            ...
            <button onClick={onClear} className="text-zinc-400 hover:text-red-500 text-xs font-medium transition-colors flex items-center gap-1 px-2 py-1 rounded hover:bg-red-50">
                <i className="fas fa-trash-alt"></i> <span className="hidden sm:inline">{t('common.clear')}</span>
            </button>
            <button onClick={onExport} className="text-zinc-400 hover:text-indigo-600 text-xs font-medium transition-colors flex items-center gap-1 px-2 py-1 rounded hover:bg-indigo-50">
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-2.5 space-y-2 bg-zinc-50/30 custom-scrollbar relative"
      >
        {(hasMoreOlder || isLoadingOlder) && (
          <div className="flex justify-center mb-2">
            <span className="text-[11px] text-zinc-400 bg-zinc-100 border border-zinc-200 rounded-full px-3 py-1">
        ...
        {filteredMessages.map((msg) => {
            ...
            const topicColor = !isOut ? getTopicColor(msg.topic).replace('bg-', 'text-') : 'text-zinc-600';
            ...
            return (
                <div key={msg.id} className="flex gap-2 animate-in fade-in duration-200">
                    <div className="flex flex-col items-center pt-1 min-w-[58px]">
                        <span className="text-[10px] font-mono text-zinc-400 tabular-nums">{formatTime(msg.timestamp)}</span>
                    ...
                    <div className={`flex-1 bg-white border border-zinc-200 rounded-lg p-2 shadow-sm hover:shadow-md transition-shadow ${isOut ? 'rounded-tl-none border-l-2 border-l-indigo-300' : 'rounded-tr-none border-r-2 border-r-emerald-300'}`}>
                        <div className="flex justify-between items-start mb-1">
                            <span className={`font-mono text-xs font-bold break-all ${topicColor} bg-zinc-50 px-1 rounded`}>
                                {msg.topic}
                            </span>
                            <div className="flex gap-2 text-[10px] text-zinc-400 font-mono">
                                <span className="bg-zinc-100 px-1 rounded">QoS {msg.qos}</span>
                                {msg.retain && <span className="text-orange-600 bg-orange-50 px-1 rounded font-bold">{t('messageLog.retain')}</span>}
                            </div>
                        </div>
                        <pre className="text-xs text-zinc-700 font-mono whitespace-pre-wrap break-words max-h-48 overflow-y-auto custom-scrollbar bg-zinc-50/50 p-1.5 rounded border border-zinc-100">
                            {msg.payload}
                        </pre>
                    </div>
                </div>
