...
const SubscriptionManager: React.FC<SubscriptionManagerProps> = ({ subscriptions, onSubscribe, onUnsubscribe, onToggleMute, isConnected }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-4 h-full flex flex-col">
      ...
      <form onSubmit={handleSubscribe} className="mb-4 flex flex-wrap items-stretch gap-2">
        <input
          ...
          className="min-w-0 flex-[1_1_12rem] px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 font-mono"
          disabled={!isConnected}
        />
        <select
          value={qos}
          onChange={(e) => setQos(Number(e.target.value) as 0|1|2)}
          className="w-20 flex-shrink-0 px-2 py-2 border border-zinc-300 rounded-lg text-sm bg-white"
          disabled={!isConnected}
        >
        ...
        <button
          ...
          className="flex-shrink-0 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-zinc-300 text-white rounded-lg transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-auto space-y-2 pr-1 custom-scrollbar">
        ...
        {subscriptions.map((sub) => (
          <div 
            key={sub.topic} 
            className={`flex items-center justify-between p-2.5 bg-zinc-50 rounded-lg border border-zinc-100 group transition-all duration-200 ${sub.muted ? 'opacity-60 grayscale bg-zinc-100' : 'hover:border-emerald-200 hover:bg-white hover:shadow-sm'}`}
          >
            <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="flex flex-col min-w-0">
                    <span className={`font-mono text-xs font-bold truncate transition-colors ${sub.muted ? 'text-zinc-500 line-through decoration-zinc-400' : 'text-zinc-700'}`}>
                        {sub.topic}
                    </span>
